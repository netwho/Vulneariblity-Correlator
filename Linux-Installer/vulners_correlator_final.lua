...
• Generated by: nmap -sV --script vulners -oX filename.xml
• Contains vulnerability data from Vulners database
• Should include CVSS scores and CVE identifiers
...
1. OPTION A - Edit the plugin file directly:
   • Edit: ~/.local/lib/wireshark/plugins/vulners_correlator_final.lua
   • Find line 5: prefs.xml_path = "/current/path"
   • Change to: prefs.xml_path = "/your/new/path/vulners_scan.xml"
   • Restart Wireshark

2. OPTION B - Use the Quick Path Update menu (recommended):
   • Use: Tools > Vulnerability Correlator > Update XML Path
   • Follow the prompts to enter new path
   • No restart required

3. OPTION C - Place file in expected location:
   • Copy your XML file to: ]] .. prefs.xml_path .. [[
   • Use: Tools > Vulnerability Correlator > Reload CVSS Data

CURRENT SETTINGS:
• XML Path: ]] .. prefs.xml_path .. [[
• File exists: ]] .. (io.open(prefs.xml_path, "r") and "✅ Yes" or "❌ No") .. [[
...
local function update_xml_path_interactive()
    if not gui_enabled() then
        log("❌ GUI not available for interactive path update")
        return false
    end
...
• Desktop: ~/Desktop/vulners_scan.xml
• Documents: ~/Documents/vulners_scan.xml  
• Downloads: ~/Downloads/vulners_scan.xml
• Project folder: /Users/walterh/vulner-correlator/samples/vulners_scan.xml

EXAMPLE VALID XML PATHS:
• /Users/walterh/Desktop/my_scan.xml
• /Users/walterh/Documents/vulnerability_scan.xml
...
═══════════════════════════════════════════════════════════════════════════════
NEED HELP? 
• For nmap: Check that your XML was generated with: nmap -sV --script vulners -oX file.xml
• For OpenVAS: Convert CSV export using: python3 helper/openvas_csv_to_xml.py input.csv output.xml
• Ensure the file contains vulnerability data (<script id="vulners"> or <vulnerability> sections)
• Verify file permissions allow reading
═══════════════════════════════════════════════════════════════════════════════]], 
        prefs.xml_path, 
        current_exists and "✅ File found and readable" or "❌ File not found or not readable"
    )
    ...
    log("📁 XML Path Configuration dialog opened")
    log("💡 To update path: Edit line 5 in plugin file, then use 'Reload CVSS Data'")
...
local function validate_xml_path(path)
    ...
    if not is_nmap and not is_openvas then
        log("⚠️  Warning: XML file may not contain supported vulnerability scan results")
        log("⚠️  Supported formats: nmap Vulners, OpenVAS (converted)")
    end
...
local function parse_vulners_xml(path)
    ...
    elseif format == "openvas" then
        ...
        log("❌ Error: Unsupported XML format detected")
        log("💡 Supported formats:")
        log("   • nmap with Vulners script: nmap -sV --script vulners -oX file.xml")
        log("   • OpenVAS (converted): Use helper/openvas_csv_to_xml.py script")
        return
    end
end
...
• Tools > Vulnerability Correlator > Configure XML Path
  - Interactive configuration helper with validation
  - Shows current file status and common locations
• Tools > Vulnerability Correlator > Find XML Files  
...
Searching for potential Vulners XML files in common locations...
  ...
  if #found_files > 0 then
    finder_content = finder_content .. "✅ FOUND FILES:\n\n"
    ...
    finder_content = finder_content .. "❌ NO VALID XML FILES FOUND\n\n"
    finder_content = finder_content .. "SEARCHED LOCATIONS:\n"
    finder_content = finder_content .. "• ~/Desktop/vulners_scan.xml\n"
    finder_content = finder_content .. "• ~/Documents/vulners_scan.xml\n"
    finder_content = finder_content .. "• ~/Downloads/vulners_scan.xml\n"
    finder_content = finder_content .. "• /Users/walterh/vulner-correlator/samples/vulners_scan.xml\n"
    finder_content = finder_content .. "• ~/vulners_scan.xml\n"
    finder_content = finder_content .. "• /tmp/vulners_scan.xml\n\n"
  ...
  finder_content = finder_content .. "• XML Path: " .. prefs.xml_path .. "\n"
  
  local current_valid, current_msg = validate_xml_path(prefs.xml_path)
  finder_content = finder_content .. "• Current File: " .. (current_valid and "✅ Valid" or "❌ " .. current_msg) .. "\n"
  ...
  log("🔍 XML File Finder completed. Found " .. #found_files .. " valid files.")
  print("\n" .. finder_content)
end, MENU_TOOLS_UNSORTED)
...
log("   • Configure XML Path - Interactive XML file path configuration")
log("   • Find XML Files - Search for XML files in common locations")
